...
},{"core-js/fn/regexp/escape":2,"core-js/shim":325,"regenerator-runtime/runtime":342}],2:[function(require,module,exports){
...
},{"sightglass":344}],344:[function(require,module,exports){
(function() {
 ...
 * Timing analytics
 *
 * Uses the browser's Timing & Performance API to log user timing to GA / GTM.
 */
'use strict';

/**
 * For sanity-checking dates in performance.timing.
 * iOS 9.1 seemed to give ridiculous numbers in the timing (avg of 300 million seconds).
 *
 * 1/1/2018 in milliseconds.
 */

var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"]) _i["return"](); } finally { if (_d) throw _e; } } return _arr; } return function (arr, i) { if (Array.isArray(arr)) { return arr; } else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); } else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); } }; }();

var SANITY_CHECK_MIN_TIME = 1514782800000;

/**
 * For sanity-checking dates in performance.timing.
 * iOS 9.1 seemed to give ridiculous numbers in the timing (avg of 300 million seconds).
 *
 * 1/1/2030 in milliseconds.
 */
var SANITY_CHECK_MAX_TIME = 1893474000000;

/**
 * For sanity-checking differences in time.
 *
 * 1 hour.
 */
var SANITY_CHECK_MAX_DIFF = 3600000;

var LABEL_SERVER_RESPONSE = 'server response';
var LABEL_DOM_INTERACTIVE = 'response -> DOM interactive';
var LABEL_DOM_COMPLETE = 'response -> DOM complete';

/**
 * Details about what in window.performance to log.
 */
var timingsToLog = [
// each item in the array is: [ [log label], [end performance.timing key], [optional start performance.timing key] ]
[LABEL_SERVER_RESPONSE, 'responseEnd', 'navigationStart'], [LABEL_DOM_INTERACTIVE, 'domInteractive', 'responseEnd'], [LABEL_DOM_COMPLETE, 'domComplete', 'responseEnd']];

/**
 * Keys are the log label from timingsToLog, values are just True.
 */
var timingsAlreadyLogged = {};

/**
 * Keys are the label, values are the time.
 */
var timingLogValues = {};

/**
 * Tries to log the given timing info.
 *
 * @param {Array} timing
 */
function logTiming(timing) {
	var _timing = _slicedToArray(timing, 3),
	    label = _timing[0],
	    timingEndKey = _timing[1],
	    timingStartKey = _timing[2];

	if (timingsAlreadyLogged[label]) {
		// Already logged this.
		return;
	}

	var timingEnd = performance.timing[timingEndKey];
	var timingStart = timingStartKey ? performance.timing[timingStartKey] : performance.timeOrigin;
	if (!timingStart || !timingEnd) {
		// Don't have the data yet.
		return;
	}

	if (!sanityCheckTimes(timingStart, timingEnd)) {
		console.log('Will not log ' + label + ' time, does not look sane (' + timingStart + ' -> ' + timingEnd + ').');
		return;
	}

	var diff = Math.round(timingEnd - timingStart);
	...
	timingLogValues[label] = diff;
	timingsAlreadyLogged[label] = true;
}

/**
 * Returns true if the given times from performance.timing looks sane.
 *
 * @param {Number} timeStart
 * @param {Number} timeEnd
 * @return {Boolean}
 */
function sanityCheckTimes(timeStart, timeEnd) {
	return sanityCheckTime(timeStart) && sanityCheckTime(timeEnd) && timeEnd >= timeStart && timeEnd - timeStart < SANITY_CHECK_MAX_DIFF;
}

/**
 * Returns true if the given time from performance.timing looks sane.
 *
 * @param {Number} time
 * @return {Boolean}
 */
function sanityCheckTime(time) {
	return time && time >= SANITY_CHECK_MIN_TIME && time < SANITY_CHECK_MAX_TIME;
}

/**
 * Tries to log all timing info.
 */
function logAllTiming() {
	timingsToLog.forEach(logTiming);

	// If all are done, call the API to store the details locally.
	if (timingsToLog.every(function (timing) {
		var _timing2 = _slicedToArray(timing, 1),
		    label = _timing2[0];

		return timingsAlreadyLogged[label];
	})) {
		logTimingLocally();
	}
}
...
/**
 * Calls the API to store all the timing info locally.
 */
function logTimingLocally() {
	var request = new XMLHttpRequest();
	request.open('POST', '/api/timing/', true);
	request.setRequestHeader('Content-Type', 'application/json');
	request.send(JSON.stringify({
		abTest: window.serverData.analytics.dataLayer.ABTest,
		serverResponse: timingLogValues[LABEL_SERVER_RESPONSE],
		domInteractive: timingLogValues[LABEL_DOM_INTERACTIVE],
		domComplete: timingLogValues[LABEL_DOM_COMPLETE]
	}));
}
...
function init() {
	...
	// We try to log the timings 3 times:
	// 1 - immediately
	// 2 - document.ready()
	// 3 - window.load()
	//
	// Depending on the timing, different things might be available at different times.
	// For example, DOM complete isn't available until window.load().
	// But if they close the window early, we want to log all info available.
	logAllTiming();
	$(document).ready(logAllTiming);
	$(window).on('load', logAllTiming);
}
...
// ----------------
//
// Pushes values to third-party analytics and advertising systems using the Google Tag Manager `dataLayer`.
'use strict';
...
var console = require('./console-fallback');
...
require('./analytics-timing');

// dataLayer Push Helper
function analyticsPush(storage, keys, value) {
	var data = {};
	...
	// support storage of multiples keys with the same value
	if (isArray(keys)) {
		forEach(keys, function iterator(key) {
			data[key] = value;
		});
	} else {
		data[keys] = value;
	...
	if (isArray(storage)) {
		...
		storage = defaults(storage, data);
	}
}

// Define groupType
//
// Conditions that determine the value of groupType
function defineGroupType(resp) {

	var setTitle = get(resp, 'title', false);

	// daily
	if (setTitle === 'The Daily') {
		return 'daily';
	}
...
// Window Load Tracking Values
$(window).on('load', function setAnalyticsValues() {

	var dataLayer = get(window, 'serverData.analytics.dataLayer', false);
	dataLayerProps = union(dataLayerProps, keys(dataLayer));

	// push values to GTM dataLayer
	if (dataLayer) {
		gtm.push(dataLayer);
	}
});

// pushState Change Tracking Values
$(window).on('mittr:updateState', function setAnalyticsValues(e, data) {

	var dataLayer = get(data, 'analytics.dataLayer', false);
	if (!dataLayer) {
		return false;
	}

	// Flush Old `dataLayer` Values
	//
	// Keep track of the rules we set, and flush out existing rules
	// when new values are set to avoid incorrectly setting values
	// from the previous article. [simoahava.com/gtm-tips](http://www.simoahava.com/gtm-tips/remember-to-flush-unused-data-layer-variables/)
	dataLayerProps = union(dataLayerProps, keys(dataLayer));
	var flushProps = difference(dataLayerProps, keys(dataLayer));
	forEach(flushProps, function flushValue(prop) {
		dataLayer[prop] = undefined;
	});

	console.info('dataLayer PUSH', dataLayer);
...
//
// Push values to a dataLayer array. Google Tag Manager
// collection custom values from this array for use with
// analytics scripts.
//
// ### `dataLayer` Keys
// * `AccountLevel`
//
// ### `insider` Keys
// * `user.subscriptions.insider.status` = `active|expired|inactive`
// * `user.subscriptions.insider.type` = `subscriber|alum|mitef|other|(null = 'f')`
// * `user.subscriptions.insider.level` = `basic|plus|premium`
// Don't need to wait for checkSession anymore.
(function () {
	// Save user session to the rivets data `model`
	var user = get(window.serverData, 'user', false);
	var dfp = get(window.serverData, 'analytics.dfpTargeting', {});

	// Save user subscription data to the rivets data `model`
	var subscription = get(user, 'subscriptions.insider', false);
	if (subscription) {

		// Set account level values: `AccountLevel` and `subscriber_type`
		var accountLevel = get(subscription, 'level', false);
		analyticsPush(gtm, ['AccountLevel'], accountLevel);
		analyticsPush(dfp, ['subscriber_type'], accountLevel);
	}

	$(document).ready(function () {
		// Not really sure when window.ga.getAll()[0].get is safe to call, hence lots of defensive checks
		// TODO figure out why this can't be called on every page.
		if (window.ga && window.ga.getAll) {
			var client = window.ga.getAll()[0];
			if (client && client.get) {
				analyticsPush(gtm, ['ClientID'], window.ga.getAll()[0].get('clientId'));
			}
		}
	});
})();

// Entryset Tracking Values
// ----------------
// Push values to Google Analytics and DFP tracking data related to entryset's:
// * `group_id`
// * `group_type`
// * `group_name`
// * `user.subscriptions.insider.status` = `active|expired|inactive`
// * `user.subscriptions.insider.type` = `subscriber|alum|mitef|other|(null = 'f')`
// * `user.subscriptions.insider.level` = `basic|plus|premium`
$(window).on('mittr:entryset', function entrysetAnalytics(e, data) {

	var dfp = get(window.serverData, 'analytics.dfpTargeting', {});

	// Set `group_*` values for DFP targeting
	analyticsPush(dfp, 'group_id', data.id);
	analyticsPush(dfp, 'group_type', data.resp.type);
	analyticsPush(dfp, 'group_name', data.resp.title.replace(/,/g, ''));
});

// Media Playback Tracking Values
// ----------------
// Push values to Google Analytics related to audio/video playback:
// * `event` = `mediaAction`
// * `media_action` = [Played|Ended|Paused] [Audio|Video]
// * `media_label` = [the value of data-event-label]
$("audio:not([autoplay]), video:not([autoplay])").on("play ended pause", function mediaAnalytics(e) {
	var target = e.target.tagName.toLowerCase().split('');
	...
	// Push values to GTM
	analyticsPush(gtm, 'media_action', type + " " + target);
	analyticsPush(gtm, 'media_label', $(this).data('event-label'));
	analyticsPush(gtm, 'event', "mediaAction");
});

// Ad-Blocker Tracking
// -------------------
// These events are fired by the SourcePoint JS, which is loaded via GTM.
// 'ads blocked' is a variable in GTM and a custom dimension in GA.
document.addEventListener('sp.blocking', function () {
	analyticsPush(gtm, 'AdsBlocked', 'yes');
});
document.addEventListener('sp.not_blocking', function () {
	analyticsPush(gtm, 'AdsBlocked', 'no');
});

}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"./analytics-timing":346,"./console-fallback":354,"lodash.defaults":328,"lodash.difference":329,"lodash.foreach":330,"lodash.get":331,"lodash.isarray":334,"lodash.isfunction":335,"lodash.keys":337,"lodash.union":339}],348:[function(require,module,exports){
(function (global){
/**
 * Data Binding
 *
 * Rivets.js is used for two-way data binding on the <body>
 */
'use strict';
...
internals.cmsToolbox = function () {
	if (get(global.serverData, 'params.id')) {
		model.rhinoUrl = global.serverData.env.rhinoUrl + '/?p=articles&e=' + global.serverData.params.id;
	}
};

/**
 * Enable dynamic ellipses for content areas
 *
 * This should be run on window.load rather than document.ready.
 * Seems to cause innacurate calculations when it's on document.ready.
 * Not sure why, possibly because the font hasn't loaded yet?
 */
internals.dotdotdot = function () {
	var $dotdotdot = $('[data-dotdotdot]');
	if ($dotdotdot.length) {
		$dotdotdot.dotdotdot({
			wrap: 'word',
			watch: true
...
internals.debug = function () {
	...
	// set color based on data attr flags
	var $el = $('[data-debug]');
	$el.each(function () {
		var obj = $(this).data('debug');
		var json = JSON.stringify(window[obj], null, 4);
		$(this).html(json);
	});
};

internals.configure = function () {
 ...
 * Master Model
 */
internals.masterModel = function () {
	...
	// bind for observe
	view = rv.bind($('[data-model="master"]'), model);
};

/**
 * Re-bind article body to master model.
 *
 * Needed after loading a new story in the reading list.
 */
internals.rebindArticleBody = function () {

	rv.bind($('.article-body'), model);
};

/**
 * Custom Data Binders
 */
internals.customBinders = function () {

	// custom binders
	rv.binders.width = function (el, value) {
		el.style.setProperty('width', value + '%');
	};

	// what be this?
	rv.binders['show-class-*'] = function (el, value) {
		if (el.showClass) {
			$(el).removeClass(el.showClass);
			delete el.showClass;
		}

		if (value) {
			$(el).addClass(this.args[0]);
			el.showClass = this.args[0];
		}
	};

	// what be this?
	rv.binders['hide-class-*'] = function (el, value) {
		if (el.hideClass) {
			$(el).removeClass(el.hideClass);
			delete el.hideClass;
		}

		if (!value) {
			$(el).addClass(this.args[0]);
			el.hideClass = this.args[0];
		}
	};

	// inline styles
	rv.binders['style-*'] = function (el, value) {
		if (value !== null) {
	...
	// readonly
	rv.binders.readonly = function (el, value) {
		el.readOnly = !!value;
	};
};

internals.customFormatters = function () {
	...
	rv.formatters.json = {
		...
		publish: function publish(string) {
			return JSON.parse(string);
		}
	};

	// time ago
	rv.formatters.date = function (value) {
		if (!value) return;
		return moment(value).fromNow();
	};

	// lowercase
	rv.formatters.lowercase = function (value) {
		if (!value) return;
		return value.toLowerCase();
	};

	// capitalize
	rv.formatters.capitalize = function (value) {
		if (!value) return;
		return value.charAt(0).toUpperCase() + value.slice(1);
	};

	// get the first letter
	rv.formatters.initial = function (value) {
		if (!value) return;
		return value.substr(0, 1);
	};

	// extract values from user.session.profile for use in the account dropdown menu
	rv.formatters.displayName = function (value) {
		if (!value) return;

		var name;
		var fallback = 'My Account';

		// format for names
		if (value.first_name && value.last_name) {
			name = value.first_name + ' ' + value.last_name.substr(0, 1) + '.';
		...
		// safety net for strange situation where
		// "[Object Object]" is provided
		if (!isString(name)) {
			name = fallback;
		}
	...
	// turn array of authors into a string with appropriate
	// editorial separators. Used on profile page in bookmark listing.
	rv.formatters.byline = function (value) {
		if (!value || typeof value === 'string' || typeof value === 'number') return;
		var authors = '';
		for (var i = 0; i < value.length; i++) {
			if (value.length >= 3 && value.length - 1 === i) {
				authors = authors + value[i].text + ', and ';
			} else if (value.length === 2 && value.length - 1 === i) {
				authors = authors + value[i].text + ' and ';
			} else if (value.length === 1) {
				authors = authors + value[i].text;
			} else {
				authors = authors + value[i].text + ', ';
			}
		}
	...
	// string or array includes
	rv.formatters.includes = function (value, search) {
 ...
 * Dropcaps
 */
internals.dropCap = function () {

	var $parent = $('.article-body:last').first();
	var $el = $('.article-body__content > p', $parent).first();
	if ($el.length <= 0) return;

	// skip dropcaps for horizontal topper layouts that are not of type standard or business-report
	// (i.e. { layout: horizontal, type:graphiti } and { layout: horizontal, type: video } )
	var $topper = $('[data-topper-layout]', $parent).first();
	var topperData = $topper.data();
	if (!topperData) {
		return;
	}
	if (topperData.topperLayout === 'horizontal' && topperData.topperType !== 'standard' && topperData.topperType !== 'business-report' && topperData.topperType !== 'technology' && topperData.topperType !== 'technologies-list') return;

	// per request of management, skip dropcaps on toppers with no images
	// (i.e. {layout: standard, type: standard} )
	if (topperData.topperLayout === 'standard' && topperData.topperType === 'standard') return;
	...
	if (text.charAt(0) != '<' && text.charAt(0) != '&') {
		...
		$el.html('<span class="dropcap">' + first + '</span>' + rest);
	}
};
 ...
 * Accent Colors
 *
 * Sets an arbitrary CSS property to an accent
 * color specified by the API (CMS controlled colors)
 *
 * Had issues with using rv-style-* inside of an rv-each-*
 * prior to launch, so the code below. (Kevin)
 */
internals.accentColors = function () {

	// only for lists with a defined color
	if (!has(model, 'listData.topper.color')) {
		return;
	}

	// CMS controlled hex colors
	var hex = model.listData.topper.color;

	// set color based on data attr flags
	var $elems = $('[data-accent-style]');
	$elems.each(function () {
		var property = $(this).data('accent-style');
		$(this).css(property, hex);
	});
};

/**
 * Show/hide annotations on click.
 *
 * Not sure if this is the best place for it, but not sure where else it should go.
 */
internals.annotations = function () {

	$(document).on('click', '.l-article-annotation', function (e) {

		if ($(e.target).hasClass('close-annotation')) {
			return true;
		}

		$('.l-article-annotation-container').remove();
		$('.l-article-annotation').removeClass('active');
		...
		var width = $(this).closest('p').css('width');

		annotation_html += '<div class="l-article-annotation-container" style="width:' + width + '">';
		annotation_html += '<div class="close-annotation">&times;</div>';
		if (icon.length != 0) {
			var cdn = serverData.env.cdnHost ? serverData.env.cdnHost : '';
			annotation_html += '<img class="annotation-icon" src="' + cdn + '/_/img/annotation-icons/' + icon + '"/>';
			annotation_html += '<div class="annotation-text">' + annotation.replace(/(?:\r\n|\r|\n)/g, '<br />') + '</div>';
		} else {
			annotation_html += '<div class="annotation-text" style="margin-left:7% !important">' + annotation.replace(/(?:\r\n|\r|\n)/g, '<br />') + '</div>';
		}

		$(this).after(annotation_html);
...
/**
 * Expands/collapses the help description when clicking the icon in the topper topic.
 *
 * Currently only used for VFTM.
 *
 * Not sure if this is the best place for it, but not sure where else it should go.
 */
internals.topperTopicHelp = function () {

	$(document).on('click', '.article-topper__topic__help-icon', function (e) {

		var $wrapper = $(e.target).parents('.article-topper__topic-help-wrapper');
		var $help = $wrapper.find('.article-topper__topic__help-icon');

		if (!$wrapper.is('.article-topper__topic-help-wrapper--expanded')) {
			// Closed. Open it.
			$wrapper.addClass('article-topper__topic-help-wrapper--expanded');
			$help.removeClass('ion-help-circled');
			$help.addClass('ion-close-circled');
		} else {
			// Opened. Close it.
			$wrapper.removeClass('article-topper__topic-help-wrapper--expanded');
			$help.removeClass('ion-close-circled');
			$help.addClass('ion-help-circled');
		}
	});
};

// initalize
$(window).on('load mittr:appendEntry', internals.dropCap.bind(internals));
$(window).on('load mittr:appendEntry', internals.accentColors.bind(internals));
$(window).on('load', internals.topperTopicHelp.bind(internals));
$(window).on('load', internals.annotations.bind(internals));
$(window).on('load', internals.dotdotdot.bind(internals));
$(window).on('mittr:bookmarksLoaded mittr:appendEntry', internals.rebindArticleBody.bind(internals));
$(document).ready(internals.init.bind(internals));

}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"../vendor/jquery.dotdotdot":361,"lodash.get":331,"lodash.has":332,"lodash.includes":333,"lodash.isstring":336,"lodash.merge":338,"moment":341,"rivets":343}],349:[function(require,module,exports){
(function (global){
/**
 * Footer
 */
...
$(document).on('ready', function () {
	...
	// Show and hide subnav list
	var toggleSubnav = function toggleSubnav() {
		$(this).parent().toggleClass('js-footer-list-is-visible');
	};

	//Add the click event to the selector for screen widths under 850. Otherwise, remove it.
	var setFooterClickEvent = function setFooterClickEvent(windowWidth) {
		if (windowWidth < 850) {
			$('.footer__subnav__list-title-span').off('click.footer').on('click.footer', toggleSubnav);
		} else {
			$('.footer__subnav__list-title-span').off('click.footer', toggleSubnav);
		}
	};

	// Find the new resized window width and set the click event again.
	$(window).on('resize', function () {
		var resizedWindowWidth = $(window).width();
		setFooterClickEvent(resizedWindowWidth);
	});

	setFooterClickEvent(windowWidth);
}); // end ready
...
internals.init = function () {
	...
	// methods on load
	this.setNavMenuClickEvent();
	this.toggleGlobalNavDropdowns();
	this.hideNavOnBodyClick();

	// event handlers
	$(window).on('resize', internals.onViewportResize.bind(internals));
	$('[data-toggle="search-form"]').off('click.search-toggle').on('click.search-toggle', internals.toggleSearch.bind(internals));
	$('#search-form').on('click', '[data-search="close"]', internals.hideSearch.bind(internals));
	$('#search-form').on('keyup', '[data-search="search-terms"]', internals.onKeyUp.bind(internals));
	$('#search-form').on('click', '[data-search="submit"]', internals.searchQuery.bind(internals));
	$('#search-form').on('submit', internals.searchQuery.bind(internals));
};

// Resize dimensions when viewport size changes
internals.onViewportResize = function () {
	var windowWidth = $(window).width();
	this.setNavMenuClickEvent(windowWidth);
	this.toggleGlobalNavDropdowns();
};

// Remove classes from body that affect the functioning of the navbar in mobile
internals.disableMobileNav = function () {
	model.nav.fixed = false;
	model.nav.open = false;
	model.readingList.menuLabel = 'Menu';
};

// mobile nav click
internals.setNavMenuClickEvent = function (windowWidth) {
	windowWidth = windowWidth || this.data.windowWidth;

	if (windowWidth < 750 || model.isTouch) {
		this.disableMobileNav();
		$('[data-mobile-menu="toggle"]').off('click.mobile-menu').on('click.mobile-menu', function (e) {
			e.preventDefault();
			var newLabel = model.mobileNav.open ? 'Menu' : 'Close';

			model.mobileNav.open = !model.mobileNav.open;
			model.mobileNav.menuLabel = newLabel;

			model.nav.open = !model.nav.open;
			model.nav.menuLabel = !model.nav.menuLabel;

			model.readingList.menuLabel = newLabel;
		});
	}
};

// Show and hide exapandable menu items in navbar.
// Used in the global navbar under "Topics +" and "More +"
internals.toggleGlobalNavDropdowns = function () {

	$('.navbar__item__link--expandable').each(function () {
		var $expandableLink = $(this);

		$expandableLink.bind('click', function (event) {
			event.preventDefault();
			$expandableLink.next().toggleClass('js-navbar-item-is-open');
			$expandableLink.parent().toggleClass('js-navbar-item-is-open');
			$('.navbar__dropdown.js-navbar-item-is-open').not($expandableLink.next()).removeClass('js-navbar-item-is-open');
			$('.navbar__item--expandable.js-navbar-item-is-open').not($expandableLink.parent()).removeClass('js-navbar-item-is-open');
		});
	});
};

// Hide the search bar
internals.hideSearch = function (e) {
	if (e) e.preventDefault();
	model.search.open = false;
	model.search.menuLabel = 'Search';
};

// Show and hide search bar
internals.toggleSearch = function (event) {

	// open search
	if (event) event.preventDefault();

	model.search.menuLabel = model.search.open ? 'Search' : 'Close';

	if (model.search.open) $('.navbar__mobile-search__input').focus();

	model.search.open = !model.search.open;
};

// Submit search query from key press
internals.onKeyUp = function (event) {
	event.preventDefault();

	// skip functionaliy on search results page
	// we run a live search on 'keyup' there
	if (has(serverData, 'endpoint') && serverData.endpoint === 'search') return;

	// ESC pressed
	if (event.keyCode === 27) this.hideSearch();

	// ENTER pressed
	else if (event.keyCode === 13) this.searchQuery();
};

// Submit search query
internals.searchQuery = function (event) {

	// for direct click events
	if (event) event.preventDefault();

	// check for serach term
	var query = $('[data-search="search-terms"]').val();
	if (!query) return false;

	window.location.href = '/search/?s=' + encodeURIComponent(query);
};

// Hide mobile navbar when clicking outside of the navbar
internals.hideNavOnBodyClick = function () {
	var self = this;
	$('.site-content').on('click', function (e) {
		if (model.mobileNav.open) {
			e.preventDefault();

			// hide nav
			model.mobileNav.open = false;
			model.mobileNav.menuLabel = 'Menu';
			model.readingList.menuLabel = 'Menu';

			self.disableMobileNav();

			// close other navs
			$('.js-dropdown-nav-item-is-open').removeClass('.js-dropdown-nav-item-is-open');
		}
	});
};
...
},{"lodash.has":332,"tappy":345}],351:[function(require,module,exports){
'use strict';
...
},{"./footer":349,"./global":350}],352:[function(require,module,exports){
(function (global){
// Base is inherited by javascript files in `public/src`.
// A module should not be included in here if it isn't required by almost every page on the website.
// Page specific javascript should go in its own file in `public/src` that requires base.
'use strict';

// Don't redefine $ in case something else already set it up and added more plugins,
// which happens on the SFG purchase forms for gifts.
...
require('./truncate');
require('./dfp');
require('./article-data-binding');
require('./body-class');
require('./analytics');
require('./article-navigation');
require('./newsletters');
require('./entrysets/links');
require('./lazy-load');

}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"./analytics":347,"./article-data-binding":348,"./article-navigation":351,"./body-class":353,"./dfp":355,"./entrysets/links":356,"./lazy-load":357,"./newsletters":358,"./truncate":359,"babel-polyfill":1}],353:[function(require,module,exports){
(function (global){
// Body Classes
// -----------------------
// Conditions for setting and updating `<body>` class attributes when
// sessions are checked or change.
'use strict';
...
internals.loggedIn = function () {
	$body.removeClass('logged-out').addClass('logged-in');
};

internals.loggedOut = function () {
	$body.removeClass('logged-in').addClass('logged-out');
};

// Debug Notices
// -----------------------
// Attach notices to the DOM when `?debug` query is active in the URL
function addDebugNotice(selector, attachFn) {
	$(selector).each(function () {
		var data = $(this).data();

		// definition list for [data] key => value
		var html = '<table class="debug-notice" data-debug-notice="' + selector + '">';
		forEach(data, function iterator(val, key) {
			html += '<tr>';
			html += '<th>' + key + '</th>';
			html += '<td>' + val + '</td>';
			html += '</tr>';
		...
		return attachFn.call(this, html);
	});
}

// Controls whether or not we're in debug mode
internals.debugMode = function () {

	// Enabled when `?debug` query exists
	if (!serverData.debug) {
		return;
	}
	...
	// rhino toppers & widgets
	addDebugNotice('[data-topper-type], [data-widget-layout], [data-widget-type]', function attachFn(html) {
		$(this).append(html);
	});

	// dfp ads
	addDebugNotice('[data-adunit]', function attachFn(html) {
		$(this).after(html);
	});
};

(function () {
	if (!has(window.serverData, 'user.id')) {
		internals.loggedOut();
	} else {
		internals.loggedIn();
	}
...
// Action on log-in action
$(document).on('mittr:login', function () {
	internals.loggedIn();
});

// Action on log-out action
$(document).on('mittr:logoutDone', function () {
	internals.loggedOut();
});

// Rivets ready event, this helps avoid a FOUT (Flash of Unstyled Text) for
// elements that are visually displayed or hidden with rivets attributes.
...
},{"lodash.foreach":330,"lodash.has":332}],354:[function(require,module,exports){
/**
 * Console Fallback
 *
 * When we don't have a console, don't break things.
 */
'use strict';
...
for (var i in methods) {
  if (typeof module.exports[methods[i]] === 'undefined') {
    module.exports[methods[i]] = fallback;
  }
}

},{}],355:[function(require,module,exports){
(function (global){
// dfp.js uses a jquery plugin, jquery.dfp.js, to configure and initialize advertisments.
// jquery.dfp.js initializes ads in divs with the class `adunit`, and also consumes data attributes
// See https://github.com/coop182/jquery.dfp.js
'use strict';

var $ = (typeof window !== "undefined" ? window['$'] : typeof global !== "undefined" ? global['$'] : null);
var get = require('lodash.get');
require('jquery.dfp');
var lastPageYOffset = -52;
var INTERSTITIAL_DELAY = 1000 * 5; // 5s
var SLIDER_DELAY_LONG = 1000 * 15; // 15s
var SLIDER_DELAY_SHORT = 1000 * 5; // 5s
var userIsFromNewsletter = /utm_medium=email/.test(window.location.search);
var roadblockTriggered = false;

var internals = {};

function loadInactiveAds() {
	var selector = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';

	var inactive = $('.adunit[data-active=\'false\']', selector);
	// The pathname check is needed on list pages because more ads can be loaded in
	// at any time so we cant stop looking for them.
	if (!inactive.length && !window.location.pathname.match(/lists/)) return;

	for (var i = 0; i < inactive.length; i++) {
		var $ad = $(inactive[i]);

		if ($ad.hasClass('ad--slider')) {
			if (window.pageYOffset < 100) continue;

			if (!roadblockTriggered) {
				loadAd($ad);
			} else {
				$ad.remove();
			}
...
function dfpOnFrame() {
	if (window.pageYOffset > lastPageYOffset + 50) {
		lastPageYOffset = window.pageYOffset;
		loadInactiveAds();
	}

	// Keep checking for ads to load
	window.requestAnimationFrame(dfpOnFrame);
}

$(document).ready(function () {
	if (window.location.search && window.location.search.match(/[\?&]nodfp\b/i)) {
		return;
	}
	...
	switch (true) {
		case Array.isArray(pathname.match(/^\/the-download\/.*$/)):
			return;
		default:
			window.requestAnimationFrame(dfpOnFrame);
	}
...
// loadAd takes a jQuery selection containing an ad and configures and initializes it
// Do not call loadAd on an ad that has already been loaded, it will cause it to refresh.
function loadAd($ad) {
	// Don't use $el.data() here, it doesn't update the DOM
	$ad.attr('data-active', true, true);

	var targeting = get(window.serverData, 'analytics.dfpTargeting');
	// If group values are set, they take precedence, remove channel
	if (targeting && (targeting.group_id || targeting.group_type || targeting.group_name)) {
	...
	$ad.dfp({
		...
		enableSingleRequest: false, // MUST be false, if you want to call $.dfp multiple times
		sizeMapping: sizeMapping(),
		setTargeting: targeting,
		afterEachAdLoaded: afterEachAdLoaded
	});
}

// `sizeMapping` defines named size maps that can be used with in combination with
// the data-size-mapping attribute to enable responsive ad sizing.
//
// Do not edit unless you've read through the documentation and understand what you're doing
//
// See https://github.com/coop182/jquery.dfp.js, https://support.google.com/dfp_premium/answer/3423562?hl=en
function sizeMapping() {
	var isInternalReferrer = document.referrer.indexOf(location.protocol + '//' + location.host) === 0;
	var isDfpPreview = window.location.search.match(/dfp-preview/);

	// Disabling all roadblock ads to comply with Chrome ad-blocking (2/15/2018).
	// Will change them to postitials.
	var subscriberRoadblock = [{
		browser: [0, 0],
		ad_sizes: []
	}];
	/*
 // To avoid being penalized by Google, we don't show mobile roadblocks to visitors
 // on the first page they visit.  We also check for the dfp-preview query param,
 // to allow the creative to be previewed in DFP.
 if (isInternalReferrer || isDfpPreview) {
 	subscriberRoadblock = [
 		{
 			browser: [0, 0],
 			ad_sizes: [[320, 480]]
 ...
 */
	...
	var fullwidthInterstitial = [{
		browser: [0, 0],
		ad_sizes: [[320, 50], [320, 100]]
	}, {
		browser: [1024, 0],
		ad_sizes: [[728, 90], [970, 90], [970, 250], [1440, 500]]
	}];

	var slider = [
	// Disabling sliders on mobile to comply with Chrome ad-blocking (2/15/2018).
	// Will change them to postitials.
	{
		browser: [0, 0],
		ad_sizes: []
	...
	var theDownloadModalLeaderboard = [{
		browser: [0, 0],
		ad_sizes: [[320, 50], [320, 100]]
	}, {
		browser: [1024, 0],
	...
	return {
		...
		'square--desktop': squareDesktop,
		'square-rectangle--desktop': squareRectangleDesktop,
		'square-rectangle--mobile': squareRectangleMobile,
		footer: footer,
		'fullwidth-interstitial': fullwidthInterstitial,
		slider: slider,
		'subscriber-roadblock': subscriberRoadblock,
		'the-download': theDownload,
		'the-download-modal-leaderboard': theDownloadModalLeaderboard
	};
}

// Hacky check to see if an ad was loaded.
// After the ad has been loaded, jQuery.dfp will add the class 'display-none' to empty ad slots
// Therefore we can check for this class to tell if the ad is loaded
function isEmpty(adunit) {
	return adunit && adunit.hasClass('display-none');
}

function initFullpageInterstitial(adunit) {
	// Delete the interstitial if the adslot wasnt filled
	// or if the user is coming from one of our newsletters
	if (isEmpty(adunit)) {
		$('#fullpage-interstitial-overlay').remove();
		return;
	}

	// Prevent body scrolling
	// Move this to a noscroll class on the body, html
	// Then remove the scroll when you close the modal
	$('html').addClass('noscroll');

	// Unhide the adunit and the interstitial overlay
	adunit.css({ visibility: 'visible' });
	$('#fullpage-interstitial-overlay').css({ visibility: 'visible' });

	$('#fullpage-interstitial-close, #fullpage-interstitial-overlay').click(function () {
		// If it's the subscriber roadblock, report it was closed.
		if (internals.subscriberRoadblock) {
			reportSubscriberRoadblockClose({
				exit: 'overlay',
				roadblock: internals.subscriberRoadblock.roadblock,
				version: internals.subscriberRoadblock.version,
				optout: internals.subscriberRoadblock.optout
			});
		} else {
			// Just close the interstitial.
			closeDfpIframe();
		}
	});

	adunit.attr('data-active', true, true);
}

function initSlider(adunit) {
	if (isEmpty(adunit)) {
		// Need to remove this button if no ad was loaded
		$('#slider-close').remove();
		return;
	}
	...
	// Gets the ad ready to display via CSS
	adunit.attr('data-loaded', true, true);

	// Close the slider when the user clicks the close button
	$('#slider-close').click(function () {
		$('#slider').remove();
	});
}

// Called after each ad is loaded. Determintes the type of ad was loaded being loaded and displays it.
function afterEachAdLoaded(adunit) {
	var slot = adunit.data('adunit');

	if (/slider/.test(slot)) {
		if (userIsFromNewsletter) {
			// If the user is coming from a newsletter, hide the roadblocks and use the short delay
			window.setTimeout(function () {
				initSlider(adunit);
			}, SLIDER_DELAY_SHORT);
		} else {
			// Otherwise, the user may get a roadblock so we should delay the slider for longer
			window.setTimeout(function () {
				if (!roadblockTriggered) {
					initSlider(adunit);
				}
	...
	} else if (/fullpage-interstitial/.test(slot) || /__subscriber\/__roadblock/.test(slot)) {
		// If the user is coming from a newsletter, we don't want to show them a roadblock.
		if (!userIsFromNewsletter) {
			window.setTimeout(function () {
				initFullpageInterstitial(adunit);
			}, INTERSTITIAL_DELAY);
		}
	} else {
		adunit.css({ visibility: 'visible' });
...
// Listen for postMessage sent from the subscriber roadblock ad.
window.addEventListener('message', function (message) {
	if (message.data.roadblock) {
		// This is called when they close the roadblock.
		reportSubscriberRoadblockClose(message.data);
	} else if (message.data.openRoadblock) {
		// This is called when the roadblock first loads.
		setSubscriberRoadblock(message.data);
	}
}, false);

// Records the current roadblock so we can accurately report it
// if it's closed via the overlay.
function setSubscriberRoadblock(data) {
	internals.subscriberRoadblock = {
		roadblock: data.openRoadblock,
		version: data.version,
		optout: data.optout
	};
}

function closeDfpIframe() {
	$('html').removeClass('noscroll');
	$('#fullpage-interstitial-overlay').remove();
}

// Called when we close the subscriber roadblock.
function reportSubscriberRoadblockClose(data) {
	// The iframe passes a URL when it's supposed to open a new window.
	// If that happens we can't close the iframe immediately or the link won't work.
	// Give it enough time to open the link before we close the iframe.
	if (data.url) {
		window.setTimeout(closeDfpIframe, 500);
	} else {
		closeDfpIframe();
	}
	...
	delete internals.subscriberRoadblock;
}

module.exports = { dfpOnFrame: dfpOnFrame, loadInactiveAds: loadInactiveAds };

}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"jquery.dfp":327,"lodash.get":331}],356:[function(require,module,exports){
(function (global){
/**
 * Hash Link Handler
 *
 * Appends hashes for entryset's to anchor links within
 * a given container. [data] attributes are used to pass
 * information from the container to each child link that
 * needs a hash. Each data attribute maps to the hash
 * routing params we have set in view-entry.js.
 *
 * Hash route: /#/set/:query/:value/
 * [data-entryset-query="id|slug"] => :query
 * [data-entryset-value="..."] => :value
 * [data-entryset-link] => identifies links within the container
 */
'use strict';
 ...
 * Event handler
 */
internals.clickHandler = function (e) {
  var data = $(e.delegateTarget).data();
  var param = 'set=' + data.entrysetValue;
  if (e.currentTarget.search) {
    e.currentTarget.search += '&' + param;
  } else {
    e.currentTarget.search = '?' + param;
  }
};
 ...
 */
module.exports = function () {
  $('[data-entryset-value]').on('click', '[data-entryset-link]', internals.clickHandler.bind(internals));
}();
...
(function () {
  if (!window.mittrDidAddLazyImgStyles) {
    var style = document.createElement('style');
    style.innerHTML = '\n    .lozad {\n      opacity: 0;\n\t\t\ttransition: opacity 0.2s ease 0.2s;\n    }\n    .lozad[data-loaded=\'true\'] {\n      opacity: 1;\n    }\n    .lozad img {\n      max-width: 100%;\n    }\n';
    document.body.appendChild(style);
    window.mittrDidAddLazyImgStyles = true;
  }
  observer.observe();
})();

},{"intersection-observer":326,"lozad":340}],358:[function(require,module,exports){
(function (global){
'use strict';
...
var internals = {
	path: '/api/newsletters/contact',
	footerForm: '.footer__newsletter-form',
	footerInput: '.footer__newsletter-input',
	footerSubmit: '.footer__newsletter-input__submit',
	footerSuccess: "<p class='footer__newsletter-success'><b>Thanks for signing up.</b> Please check your email to confirm.</p>",
	footerError: '.footer__newsletter-form-error',
	footerTrackingLabel: 'footer area location',
	hpDailyForm: '.hp-daily__form',
	hpDailyInput: '.hp-daily__form__input',
	hpDailySubmit: '.hp-daily__form__submit',
	hpDailySuccess: "<div class='hp-daily__form-success'><p><b>Thanks for signing up.</b></p><p>Please check your email to confirm your subscription.</p></div>",
	hpDailyError: '.hp-daily__form-error',
	hpDailyDesc: '.hp-daily__form__desc',
	hpDailyTrackingLabel: 'homepage top section',
	downloadForm: '.download__form',
	downloadInput: '.download__form__input',
	downloadSubmit: '.download__form__submit',
	downloadSuccess: "<div class='download__form-success'><p><b>Thanks for signing up.</b></p><p>Please check your email to confirm your subscription.</p></div>",
	downloadError: '.download__form-error',
	downloadDesc: '.download__form__desc',
	downloadTrackingLabel: 'download top section',
	downloadPromoForm: '.pricing__download__form',
	downloadPromoInput: '.pricing__download__form__input',
	downloadPromoSubmit: '.pricing__download__form__submit',
	downloadPromoSuccess: "<div class='pricing__download__form-success'><p class='pricing__download__form-success__heading'>Thanks for signing up.</p><p class='pricing__download__form-success__message'>Please check your email to confirm your subscription.</p></div>",
	downloadPromoError: '.pricing__download__form-error',
	downloadPromoDesc: '.pricing__download__form__desc',
	downloadPromoTrackingLabel: 'insider pricing promo',
	dailyForm: '.daily__form',
	dailyInput: '.daily__form__input',
	dailySubmit: '.daily__form__submit',
	dailySuccess: "<p class='daily__form-success'><b>Thanks for signing up.</b><br>Please check your email to confirm.</p>",
	dailyError: '.daily__form-error',
	dailyDesc: '.daily__form__desc',
	dailyTrackingLabel: 'footer area location',
	articleForm: '.article__newsletter-form',
	articleInput: '.cta__email',
	articleSubmit: '.cta__submit',
	articleSuccess: '.cta__thanks',
	articleError: '.cta__error',
	articleDesc: '',
	articleTrackingLabel: 'article widget left',
	insiderGroupOnboardForm: '.insider-group-specific__download-form',
	insiderGroupOnboardInput: '.insider-group-specific__download-input',
	insiderGroupOnboardSubmit: '.insider-group-specific__download-submit',
	insiderGroupOnboardSuccess: "<div class='insider-group-specific__download-success'><p><b>Thanks for signing up.</b></p><p>Please check your email to confirm your subscription.</p></div>",
	insiderGroupOnboardError: '.insider-group-specific__download-error',
	// @TODO: What is this?
	insiderGroupOnboardDesc: '.insider-group-specific__download-desc',
	insiderGroupOnboardTrackingLabel: 'insider group onboard'
};

internals.init = function init() {
	$(internals.footerForm).submit(function (e) {
		var selections = new Array();
		var listName = document.getElementsByClassName('footer__newsletter-form')[0].dataset.listName;
		internals.footerTrackingLabel = internals.footerTrackingLabel + ': ' + listName;
		selections.push('selections[' + listName + ']');
		internals.onSubmit(e, internals.footerSubmit, internals.footerInput, selections, internals.footerError, internals.footerSuccess, null, internals.footerTrackingLabel);
	});

	$(internals.dailyForm).submit(function (e) {
		var selections = new Array();
		selections.push('selections[download]');
		internals.onSubmit(e, internals.dailySubmit, internals.dailyInput, selections, internals.dailyError, internals.dailySuccess, internals.dailyDesc, internals.dailyTrackingLabel);
	});

	$(internals.hpDailyForm).submit(function (e) {
		var selections = new Array();
		selections.push('selections[download]');
		internals.onSubmit(e, internals.hpDailySubmit, internals.hpDailyInput, selections, internals.hpDailyError, internals.hpDailySuccess, internals.hpDailyDesc, internals.hpDailyTrackingLabel);
	});

	$(internals.downloadForm).submit(function (e) {
		var selections = new Array();
		selections.push('selections[download]');
		internals.onSubmit(e, internals.downloadSubmit, internals.downloadInput, selections, internals.downloadError, internals.downloadSuccess, internals.downloadDesc, internals.downloadTrackingLabel);
	});

	$(internals.downloadPromoForm).submit(function (e) {
		var selections = new Array();
		selections.push('selections[download]');
		internals.onSubmit(e, internals.downloadPromoSubmit, internals.downloadPromoInput, selections, internals.downloadPromoError, internals.downloadPromoSuccess, internals.downloadPromoDesc, internals.downloadPromoTrackingLabel);
	});

	$(internals.articleForm).submit(function (e) {
		var newsletter = $('.cta__button', internals.articleForm).attr('cta-type');
		var selections = new Array();
		selections.push('selections[' + newsletter + ']');
		internals.onSubmitArticle(e, internals.articleSubmit, internals.articleInput, selections, internals.articleError, internals.articleSuccess, internals.articleDesc, internals.articleTrackingLabel);
	});

	$(internals.insiderGroupOnboardForm).submit(function (e) {
		var selections = new Array();
		selections.push('selections[download]');
		internals.onSubmit(e, internals.insiderGroupOnboardSubmit, internals.insiderGroupOnboardInput, selections, internals.insiderGroupOnboardError, internals.insiderGroupOnboardSuccess, internals.insiderGroupOnboardDesc, internals.insiderGroupOnboardTrackingLabel);
	});
};

internals.onSubmit = function onSubmit(e, submit, email, selections, error, success, desc, trackingLabel) {
	e.preventDefault();

	// Remove any error message from a previous attempt
	// Use a space here so that the height of the element doesn't change
	$(error).html('&nbsp;');
	...
	// Add the selections to the params list
	$.each(selections, function (key, value) {
		params[value] = 'enable';
	});

	$.post(internals.path, params, function (resp) {

		if (resp.errors) {

			// Enable the submit button and focus the input
			// so the user can try again
			$(email).focus();
			$(submit).prop('disabled', false);

			// Display the error
			$(error).text(resp.errors[0].message).css('display', 'block');
		} else if (resp.contact) {

			// Successfully subscribed
			$(submit).remove();

			// I hate this stupid overcomplicated form
			if (desc) {
				$(desc).remove();
			}

			// Success message
			$(email).replaceWith(success);
		}
	});
...
internals.onSubmitArticle = function onSubmit(e, submit, email, selections, error, success, desc, trackingLabel) {
	e.preventDefault();
...
(function (global){
/**
 * Shortened String Expand/Collapse
 *
 * Compliments a custom Nunjucks filter |shorten(...)
 * (Renamed from 'truncate' because Nunjucks already has a built-in truncate filter)
 */
'use strict';
...
$(function () {
	...
	$(document).on('click', '[data-truncate="trigger"]', function (e) {
		e.preventDefault();

		var $trigger = $(this).parent('.truncate__trigger');
		$trigger.addClass('truncate__trigger--active');

		var $extended = $trigger.next('[data-truncate="extended"]');
		$extended.addClass('truncate__extended--active');
	});
});
...
// Default contains the bare minimum javascript you need to use the site.
require('../modules/base');

},{"../modules/base":352}],361:[function(require,module,exports){
'use strict';
